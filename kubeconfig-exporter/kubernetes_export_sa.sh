...
if [[ $# -lt 2 ]]; then
  ...
  echo "ex: sh ./kubeconfig-exporter/kubernetes_export_sa.sh cd-user devtroncd"
  exit 1
fi
...
get_secret_name_from_secret() {
    ...
    SECRET_NAME=$( microk8s kubectl get secret "${SERVICE_ACCOUNT_NAME}" --namespace="${NAMESPACE}" -o=jsonpath={.metadata.name})
    echo "Secret name: ${SECRET_NAME}"
}

get_secret_name_from_service_account() {
    echo -e "\\nGetting secret of service account ${SERVICE_ACCOUNT_NAME} on ${NAMESPACE}"
    SECRET_NAME=$( microk8s kubectl get sa "${SERVICE_ACCOUNT_NAME}" --namespace="${NAMESPACE}" -o=jsonpath='{.secrets[*].name}')
    echo "Secret name: ${SECRET_NAME}"
}

extract_ca_crt_from_secret() {
    echo -e -n "\\nExtracting ca.crt from secret..."
     microk8s kubectl get secret --namespace "${NAMESPACE}" "${SECRET_NAME}" -o=jsonpath="{.data.ca\.crt}"| base64 --decode > "${TARGET_FOLDER}/ca.crt"
    printf "done"
}

get_user_token_from_secret() {
    echo -e -n "\\nGetting user token from secret..."
    TOKEN=$( microk8s kubectl get secret --namespace "${NAMESPACE}" "${SECRET_NAME}" -o=jsonpath={.data.token}|base64 --decode)
    printf "done"
}
...
set_kube_config_values() {
    ...
    CLUSTER_NAME=$( microk8s kubectl config get-contexts "$context" | awk '{print $3}' | tail -n 1)
    echo "Cluster name: ${CLUSTER_NAME}"

    SERVER_URL=$( microk8s kubectl config view \
    -o jsonpath="{.clusters[?(@.name == \"${CLUSTER_NAME}\")].cluster.server}")

    # Set up the config
    echo -e "\\nPreparing k8s-${SERVICE_ACCOUNT_NAME}-${NAMESPACE}-conf"
    echo -n "Setting a cluster entry in kubeconfig..."
     microk8s kubectl config set-cluster "${CLUSTER_NAME}" \
    ...
    echo -n "Setting token credentials entry in kubeconfig..."
     microk8s kubectl config set-credentials \
    "${SERVICE_ACCOUNT_NAME}-${NAMESPACE}-${CLUSTER_NAME}" \
    --kubeconfig="${KUBECFG_FILE_NAME}" \
    --token="${TOKEN}"

    echo -n "Setting a context entry in kubeconfig..."
     microk8s kubectl config set-context \
    "${SERVICE_ACCOUNT_NAME}-${NAMESPACE}-${CLUSTER_NAME}" \
    --kubeconfig="${KUBECFG_FILE_NAME}" \
    ...
    echo -n "Setting the current-context in the kubeconfig file..."
     microk8s kubectl config use-context "${SERVICE_ACCOUNT_NAME}-${NAMESPACE}-${CLUSTER_NAME}" \
    --kubeconfig="${KUBECFG_FILE_NAME}"
}

CLIENT_VERSION=$(microk8s kubectl version -o json | awk -F '"' '/"clientVersion"/ {getline; getline; print $4}' | cut -d '.' -f 2)
CLIENT_VERSION=$(echo "${CLIENT_VERSION%+}")
echo "$CLIENT_VERSION"
if [[ $CLIENT_VERSION -gt 27 ]]
then
    VERSION=$( microk8s kubectl version | awk '/Server Version: /{print $3}' | cut -d '.' -f 2 )
    VERSION=$(expr $VERSION)
else
    VERSION=$( microk8s kubectl version --short | awk '/Server Version: /{print $3}' | cut -d '.' -f 2 )
    VERSION=$(expr $VERSION)
fi
...
echo -e "\\nAll done! Test with:"
echo "KUBECONFIG=${KUBECFG_FILE_NAME} microk8s kubectl get pods"
...
KUBECONFIG=${KUBECFG_FILE_NAME} microk8s kubectl get pods
...
if [[ ! -z "${devtron_endpoint}" ]] && [[ ! -z "${devtron_api_token}" ]] && [[ ! -z "$cluster_name" ]]; then
    ...
    echo "Please provide all required inputs to add cluster on devtron"\
